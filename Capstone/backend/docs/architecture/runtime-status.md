...
| Items | Implemented | `/items`, `/items/{id}`, `/items/{id}/restore` | `qty` read-only; dapur read-only pada GET list/show; unit write pakai nama; delete bersifat soft delete; restore bersifat idempotent |
| Stock Transactions | Implemented | `/stock-transactions`, `/stock-transactions/{id}`, `/stock-transactions/{id}/details`, `/stock-transactions/{id}/submit`, `/stock-transactions/{id}/cancel`, `/submit-revision`, `/approve`, `/reject`, `/stock-transactions/direct-corrections` | Transaksi stok adalah satu-satunya jalur mutasi stok; revision workflow adalah domain flow inti; direct stock correction tersedia untuk admin; **tidak ada DELETE route**; draft lifecycle mendukung workflow BASAH OUT |
| Menu & Nutrition | Implemented | `/menus`, `/menu-dishes`, `/menu-schedules`, `/menu-calendar`, `/dishes`, `/dishes/{id}/{deactivate,reactivate}`, `/dish-compositions` | Siklus menu 1-11; package header immutable; menu-dishes (slot assignment) supports full CRUD (POST/PUT/DELETE); deactivation detaches slots; reactivation allows new assignments; delete requires inactive detached state; resolver order: Feb 29 -> Paket 9, day 31 -> Paket 11, manual override, lalu default pattern |
...
| Audit Logs | Implemented | `/audit-logs`, `/audit-logs/types`, `/audit-logs/summary` | Admin-only audit trail read endpoint; filter metadata dan summary aggregate tersedia; audit rows juga persisted from stock, SPK override, and opname write flows |
| Notifications | Implemented | `/notifications`, `/notifications/{id}/read`, `/notifications/read-all`, `/notifications/{id}` | Notifikasi sistem self-scoped per user; mendukung filter, sorting, pagination; admin/gudang/dapur dapat mengakses notifikasi sendiri |
| Stock Snapshots | Implemented | `/stock-snapshots`, `/stock-snapshots/current` | Point-in-time snapshot qty seluruh item; digunakan untuk monthly stock export dan audit reconciliation |
...
|---|---|---|---|---|
| `items` | `GET /api/v1/items`, `POST /api/v1/items`, `GET /api/v1/items/{id}`, `PUT /api/v1/items/{id}`, `DELETE /api/v1/items/{id}`, `PATCH /api/v1/items/{id}/restore` | read: `admin,dapur,gudang`; write: `admin,gudang`; delete/restore: `admin` | `Items` -> `ItemManagementService` | `sdk.items.list`, `sdk.items.get`, `sdk.items.create`, `sdk.items.update`, `sdk.items.delete`, `sdk.items.restore` |
...
| `users` | `GET /api/v1/users`, `POST /api/v1/users`, `GET /api/v1/users/{id}`, `PUT /api/v1/users/{id}`, `PATCH /api/v1/users/{id}/activate`, `PATCH /api/v1/users/{id}/deactivate`, `PATCH /api/v1/users/{id}/password`, `DELETE /api/v1/users/{id}`, `PATCH /api/v1/users/{id}/restore` | all: `admin` | `Users` -> `UserManagementService` | `sdk.users.list`, `sdk.users.get`, `sdk.users.create`, `sdk.users.update`, `sdk.users.activate`, `sdk.users.deactivate`, `sdk.users.changePassword`, `sdk.users.delete`, `sdk.users.restore` |
| `daily-patients` | `GET /api/v1/daily-patients`, `GET /api/v1/daily-patients/{service_date}`, `POST /api/v1/daily-patients`, `PUT /api/v1/daily-patients/{id}` | GET: `admin,dapur,gudang`; POST/PUT: `admin,gudang` | `DailyPatients` -> `DailyPatientService` | `sdk.dailyPatients.list`, `sdk.dailyPatients.get`, `sdk.dailyPatients.create`, `sdk.dailyPatients.update` |
| `spk` | `GET /api/v1/spk/basah/menu-calendar`, `POST /api/v1/spk/basah/operational-stock-preview`, `POST /api/v1/spk/basah/generate`, `GET /api/v1/spk/basah/history`, `GET /api/v1/spk/basah/history/{id}`, `POST /api/v1/spk/basah/history/{id}/override`, `POST /api/v1/spk/basah/history/{id}/post-stock`, `GET /api/v1/spk/kering-pengemas/menu-calendar`, `POST /api/v1/spk/kering-pengemas/generate`, `GET /api/v1/spk/kering-pengemas/history`, `GET /api/v1/spk/kering-pengemas/history/{id}`, `POST /api/v1/spk/kering-pengemas/history/{id}/override`, `POST /api/v1/spk/kering-pengemas/history/{id}/post-stock`, `GET /api/v1/spk/stock-in-prefill/{id}` | full SPK flow: `admin,dapur,gudang`; post-stock finalization: `admin,gudang` | `SpkBasah` -> `SpkBasahGenerationService`/`SpkStockPostingService`; `SpkKeringPengemas` -> `SpkKeringPengemasGenerationService`/`SpkStockPostingService` | `sdk.spk.basahMenuCalendar`, `sdk.spk.operationalStockPreview`, `sdk.spk.generateBasah`, `sdk.spk.listBasah`, `sdk.spk.getBasah`, `sdk.spk.overrideBasah`, `sdk.spk.postBasahStock`, `sdk.spk.keringPengemasMenuCalendar`, `sdk.spk.generateKeringPengemas`, `sdk.spk.listKeringPengemas`, `sdk.spk.getKeringPengemas`, `sdk.spk.overrideKeringPengemas`, `sdk.spk.postKeringPengemasStock`, `sdk.spk.stockInPrefill` |
| `stock-opnames` | `POST /api/v1/stock-opnames`, `PUT /api/v1/stock-opnames/{id}`, `GET /api/v1/stock-opnames/{id}`, `POST /api/v1/stock-opnames/{id}/submit`, `POST /api/v1/stock-opnames/{id}/approve`, `POST /api/v1/stock-opnames/{id}/reject`, `POST /api/v1/stock-opnames/{id}/post` | create/update/show/submit: `admin,gudang`; approve/reject/post: `admin` | `StockOpnames` -> `StockOpnameService` | `sdk.stockOpnames.create`, `sdk.stockOpnames.update`, `sdk.stockOpnames.get`, `sdk.stockOpnames.submit`, `sdk.stockOpnames.approve`, `sdk.stockOpnames.reject`, `sdk.stockOpnames.post` |
| `stock-transactions` | `GET /api/v1/stock-transactions`, `POST /api/v1/stock-transactions`, `GET /api/v1/stock-transactions/{id}`, `GET /api/v1/stock-transactions/{id}/details`, `PUT /api/v1/stock-transactions/{id}`, `POST /api/v1/stock-transactions/{id}/submit`, `POST /api/v1/stock-transactions/{id}/cancel`, `POST /api/v1/stock-transactions/direct-corrections`, `POST /api/v1/stock-transactions/{id}/submit-revision`, `POST /api/v1/stock-transactions/{id}/approve`, `POST /api/v1/stock-transactions/{id}/reject` | read/list: `admin,gudang`; create/update/submit/cancel: `admin,gudang`; revision submit: `admin,gudang`; approve/reject/direct-corrections: `admin` | `StockTransactions` -> `StockTransactionService` | `sdk.stockTransactions.list`, `sdk.stockTransactions.get`, `sdk.stockTransactions.getDetails`, `sdk.stockTransactions.create`, `sdk.stockTransactions.updateDraft`, `sdk.stockTransactions.submitDraft`, `sdk.stockTransactions.cancelDraft`, `sdk.stockTransactions.submitRevision`, `sdk.stockTransactions.approve`, `sdk.stockTransactions.reject`, `sdk.stockTransactions.directCorrection` |
| `audit-logs` | `GET /api/v1/audit-logs`, `GET /api/v1/audit-logs/types`, `GET /api/v1/audit-logs/summary` | `admin` | `AuditLogs` -> `AuditLogService` | `sdk.auditLogs.list`, `sdk.auditLogs.types`, `sdk.auditLogs.summary` |
| `notifications` | `GET /api/v1/notifications`, `POST /api/v1/notifications/{id}/read`, `POST /api/v1/notifications/read-all`, `DELETE /api/v1/notifications/{id}`, `DELETE /api/v1/notifications` | authenticated token user (`admin`, `dapur`, `gudang`) — self-scoped | `Notifications` -> `NotificationService` | `sdk.notifications.list`, `sdk.notifications.markAsRead`, `sdk.notifications.markAllAsRead`, `sdk.notifications.delete`, `sdk.notifications.deleteAll` |
| `stock-snapshots` | `GET /api/v1/stock-snapshots`, `POST /api/v1/stock-snapshots`, `GET /api/v1/stock-snapshots/current` | GET: `admin,gudang`; POST: `admin,gudang`; current: `admin,dapur,gudang` | `StockSnapshots` -> `StockSnapshotService` | `sdk.stockSnapshots.list`, `sdk.stockSnapshots.take`, `sdk.stockSnapshots.current` |
...
| `dishes` | `GET /api/v1/dishes`, `GET /api/v1/dishes/{id}`, `POST /api/v1/dishes`, `PUT /api/v1/dishes/{id}`, `DELETE /api/v1/dishes/{id}`, `PATCH /api/v1/dishes/{id}/deactivate`, `PATCH /api/v1/dishes/{id}/reactivate` | GET: `admin,dapur,gudang`; POST/PUT/DELETE/PATCH: `admin,dapur` | `Dishes` | `sdk.dishes.list`, `sdk.dishes.get`, `sdk.dishes.create`, `sdk.dishes.update`, `sdk.dishes.delete`, `sdk.dishes.deactivate`, `sdk.dishes.reactivate` |
