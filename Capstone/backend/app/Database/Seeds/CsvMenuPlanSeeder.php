...
class CsvMenuPlanSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!file_exists($jsonPath)) {
            ...
            $jsonPath =
                ROOTPATH . "../../gudang-app/src/data/menu-csv-plan.json";
        }

        if (!file_exists($jsonPath)) {
            throw new RuntimeException(
                "CSV Menu Plan JSON file not found at: {$jsonPath}",
            );
        }
        ...
        if ($plan === null) {
            throw new RuntimeException(
                "Invalid JSON format in menu-csv-plan.json",
            );
        }
        ...
        foreach ($nameAliases as $jsonName => $dbName) {
            ...
            if (
                !isset($itemLookup[$normalized]) &&
                isset($itemLookup[$this->normalizeName($dbName)])
            ) {
                $itemLookup[$normalized] =
                    $itemLookup[$this->normalizeName($dbName)];
            }
        }
        ...
        foreach ($plan["recipes"] as $recipe) {
            ...
            foreach ($recipe["ingredients"] as $ingredient) {
                ...
                if ($itemId === null) {
                    ...
                    $itemId = $this->dynamicallyCreateItem(
                        $itemName,
                        $ingredient["unit"] ?? "gr",
                        $categoryLookup,
                        $unitLookup,
    ...
    private function dynamicallyCreateItem(
        ...
        if ($info === null) {
            $isBasah = preg_match(
                "/(daging|ayam|ikan|sayur|tahu|tempe|telur|bawang|cabe|tomat|buah|pisang|melon|pepaya|semangka)/i",
                $itemName,
            );
        ...
        $unitBaseId =
            $unitLookup[$unitBaseName] ?? ($unitLookup["gram"] ?? null);
        $unitConvertId =
            $unitLookup[$unitConvertName] ?? ($unitLookup["pcs"] ?? null);
