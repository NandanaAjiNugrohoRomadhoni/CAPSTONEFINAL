...
class FixMenuDishesUniqueness extends Migration
{
    public function up()
    {
        ...
        if (!$isSQLite) {
            ...
            foreach ($indices as $index) {
                try {
                    $this->db->query("ALTER TABLE menu_dishes DROP INDEX {$index}");
                } catch (\Exception $e) {
            ...
            try {
                $this->db->query("ALTER TABLE menu_schedules DROP INDEX day_of_month");
            } catch (\Exception $e) {
    ...
    public function down()
    {
        ...
        if (!$isSQLite) {
            ...
            try {
                $this->db->query("ALTER TABLE menu_schedules DROP INDEX idx_day_of_month");
            } catch (\Exception $e) {}
            
            try {
                $this->db->query("ALTER TABLE menu_schedules ADD UNIQUE KEY day_of_month (day_of_month)");
            } catch (\Exception $e) {}
        }
    }
}
