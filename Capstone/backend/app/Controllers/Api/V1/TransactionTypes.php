 ...
 * Transaction Types
 *
 * Read-only inventory lookup resource for stock transaction type resolution.
 */
class TransactionTypes extends BaseController
{
     ...
     *     @OA\Parameter(name="q", in="query", description="Primary text search term. If q and search are both present, q wins.", @OA\Schema(type="string", example="RET")),
     *     @OA\Parameter(name="search", in="query", description="Fallback text search term when q is absent.", @OA\Schema(type="string", example="OUT")),
     ...
     *     @OA\Response(response=200, description="Active transaction type collection.", @OA\JsonContent(ref="#/components/schemas/TransactionTypeCollectionResponse")),
    ...
    public function index(): ResponseInterface
    {
        ...
        $builder = $this->transactionTypeModel->builder();
        $builder->where('transaction_types.deleted_at', null);

        if ($search !== '') {
            $builder->like('transaction_types.name', $search);
        }

        $this->applyDateRange($builder, 'transaction_types.created_at', $queryParams['created_at_from'] ?? null, $queryParams['created_at_to'] ?? null);
        $this->applyDateRange($builder, 'transaction_types.updated_at', $queryParams['updated_at_from'] ?? null, $queryParams['updated_at_to'] ?? null);

        $builder->orderBy('transaction_types.' . $sortBy, $sortDir);
        if ($sortBy !== 'id') {
            $builder->orderBy('transaction_types.id', 'ASC');
        }
