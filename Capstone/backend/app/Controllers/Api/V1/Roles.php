...
class Roles extends BaseController
{
    ...
    public function index(): ResponseInterface
    {
        ...
        $builder->where('roles.deleted_at', null);

        if ($search !== '') {
            $builder->like('roles.name', $search);
        }

        $this->applyDateRange($builder, 'roles.created_at', $queryParams['created_at_from'] ?? null, $queryParams['created_at_to'] ?? null);
        $this->applyDateRange($builder, 'roles.updated_at', $queryParams['updated_at_from'] ?? null, $queryParams['updated_at_to'] ?? null);

        $builder->orderBy('roles.' . $sortBy, $sortDir);
        if ($sortBy !== 'id') {
