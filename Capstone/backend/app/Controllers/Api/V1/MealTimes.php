 ...
 * Meal Times
 *
 * Read-only menu-planning lookup resource for meal time reads.
 */
class MealTimes extends BaseController
{
     ...
     *     @OA\Parameter(name="paginate", in="query", description="Set to false or 0 to return all rows while keeping the same envelope with meta.paginated=false.", @OA\Schema(type="string", enum={"true","false","1","0"}, example="false")),
     *     @OA\Parameter(name="q", in="query", description="Primary text search term. If q and search are both present, q wins.", @OA\Schema(type="string", example="Pag")),
     *     @OA\Parameter(name="search", in="query", description="Fallback text search term when q is absent.", @OA\Schema(type="string", example="Sore")),
