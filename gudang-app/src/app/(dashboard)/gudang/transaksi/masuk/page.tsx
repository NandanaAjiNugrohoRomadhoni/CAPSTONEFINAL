...
export default function BarangMasukPage() {
  ...
  async function handleConfirmPrefill() {
    ...
    try {
      let details: Array<{ item_id: number; qty: number }> = [];
      try {
        ...
        // Fallback untuk role gudang: bangun data prefill dari detail SPK history.
        if (activeTab === "basah") {
          const detailResponse = await sdk.spk.getBasah(selectedSpkId);
          const recommendations = detailResponse.data.items ?? [];
          details = recommendations.map((item) => ({
            item_id: item.item_id,
            qty: Number(item.final_recommended_qty ?? item.system_recommended_qty ?? item.required_qty ?? 0),
          }));
        } else {
          const detailResponse = await sdk.spk.getKeringPengemas(selectedSpkId);
          const recommendations = detailResponse.data.items ?? [];
          details = recommendations.map((item) => ({
            item_id: item.item_id,
            qty: Number(item.final_recommended_qty ?? item.system_recommended_qty ?? item.required_qty ?? 0),
                ...
                return (
                  <div key={row.id} className="grid grid-cols-11 items-center gap-3 border-t px-4 py-3">
                    ...
                    <div className="col-span-1 flex justify-center">
                      <button
                        className="flex justify-center rounded-lg p-2 text-red-500 transition hover:bg-red-50"
                        onClick={() =>
                          setRows((current) =>
