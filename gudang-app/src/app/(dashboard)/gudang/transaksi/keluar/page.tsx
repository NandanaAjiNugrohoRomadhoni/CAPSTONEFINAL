...
export default function BarangKeluarPage() {
  ...
  useEffect(() => {
    ...
    async function loadSavedTransaction() {
      try {
        ...
        const savedBasahTransaction = await getLatestBasahTransaction(
          serviceDate,
          outTypeId,
        );
    ...
    try {
      ...
      openAlert(
        ...
        message.toLowerCase().includes("insufficient permissions")
          ? "Akses gudang untuk endpoint rekomendasi masih ditolak oleh server yang sedang berjalan. Pastikan backend sudah memakai update route terbaru yang mengizinkan role gudang pada POST /api/v1/spk/basah/operational-stock-preview."
          : message,
      );
    } finally {
      setValidating(false);
  ...
  async function resolveOutTypeId() {
    const transactionTypeResponse = await sdk.transactionTypes.list({ paginate: false, search: "OUT", sortBy: "name", sortDir: "ASC" });
    const outTypeId = transactionTypeResponse.data.find((row) => row.name.toUpperCase() === "OUT")?.id;
    if (!outTypeId) {
