          ...
          <div className="mb-4 flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">
                Diambil dari laporan evaluasi periode berjalan
              </p>
            </div>
          </div>
                ...
                {evaluationRows.slice(0, 5).map((row) => (
                  <tr key={String(row.spk_id)} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {formatQuantity(Number(row.planned_qty ?? 0), "kg")}
                    </td>
                    <td className="px-4 py-3 text-gray-600">
                      {formatQuantity(Number(row.realization_qty ?? 0), "kg")}
                    </td>
                  </tr>
                ))}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <SummaryBox
              tone="green"
              label="Item Aktif"
              value={formatNumber(Number(stockSummary.active_items ?? 0))}
            />
            <SummaryBox
              tone="blue"
              label="Total Item"
              value={formatNumber(Number(stockSummary.total_items ?? 0))}
            />
            <SummaryBox tone="red" label="Stok Habis" value={formatNumber(criticalCount)} />
            <SummaryBox
              tone="yellow"
              label="Total Qty"
              value={formatQuantity(Number(stockSummary.total_stock_qty ?? 0))}
            />
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            ...
            <SummaryRow
              label="Total Item"
              value={formatNumber(Number(dashboard.dry_stock_status?.total_items ?? 0))}
            />
            <SummaryRow
              label="Stok Nol"
              value={formatNumber(Number(dashboard.dry_stock_status?.zero_stock_items ?? 0))}
            />
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            ...
            <SummaryRow
              label="SPK Basah"
              value={
                dashboard.latest_spk_history?.basah?.id
                  ? `#${dashboard.latest_spk_history.basah.id}`
                  : "-"
              }
            />
