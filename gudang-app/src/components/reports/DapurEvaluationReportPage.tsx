...
export default function DapurEvaluationReportPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(
            loadError instanceof Error ? loadError.message : "Gagal memuat laporan evaluasi.",
          );
        }
      } finally {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {formatCompactDate(String(row.calculation_date ?? ""))}
                    </td>
                    <td className="px-6 py-4">
                      {formatNumber(Number(row.planned_qty ?? 0), 2)} kg
                    </td>
                    <td className="px-6 py-4">
                      {formatNumber(Number(row.realization_qty ?? 0), 2)} kg
                    </td>
                    <td
                      className={`px-6 py-4 ${
                        variance < 0
      ...
      <SurfaceCard className="overflow-hidden px-6 py-5">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-base font-semibold text-[#16213E]">
            Laporan Evaluasi 4 SPK Terakhir
          </h3>
          <button
